...
export default function AdditionalGroupAssignmentForm({
    userId,
    portfolioId,
}: {
    userId: number;
    ...
    useEffect(() => {
        async function fetchOptions() {
            const res = await fetch(
                `/api/available/group?userId=${userId}&portfolioId=${portfolioId}`
            );
            if (res.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            ...
            const data = await res.json();
            console.error("Group assignment failed", data.error);
        }
    };

    return (
        <form onSubmit={handleSubmit} className="border p-4 rounded space-y-4">
            <h3 className="text-xl font-bold">Assign Group Permission</h3>
